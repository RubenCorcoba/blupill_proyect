...
static void transferSpi2DmaBegin(void)
{
	SPI2->CR2=0; 
	DMA1_Channel5->CCR=0;
...
static void transferSpi2DmaTransfer(const uint8_t *source,uint8_t *dest,uint16_t len)
{
	static int dummy_dst[1];
	static const int dummy_src[1]={0};

	while(SPI2->SR & SPI_SR_BSY);// Espera puerto listo
	(void)SPI2->DR;
    (void)SPI2->SR; // limpia posible condicion overrun
	if(source){
		DMA1_Channel5->CMAR=(uint32_t)source;
	}else{
		DMA1_Channel5->CMAR=(uint32_t)dummy_src;
	}
	DMA1_Channel5->CNDTR=len;
	if(dest)
		DMA1_Channel4->CMAR=(uint32_t)dest;
	else
		DMA1_Channel4->CMAR=(uint32_t)dummy_dst;
	DMA1_Channel4->CNDTR=len;

	DMA1_Channel4->CCR=(dest ? DMA_CCR_MINC:0UL) | DMA_CCR_EN;
	DMA1_Channel5->CCR=(source ? DMA_CCR_MINC:0UL) | DMA_CCR_DIR |DMA_CCR_EN ;//0x91;

	while(DMA1_Channel5->CNDTR || DMA1_Channel4->CNDTR);
	while(SPI2->SR & SPI_SR_BSY);

	DMA1_Channel5->CCR=0;
	DMA1_Channel4->CCR=0;
}
static void transferSpi2DmaEnd(void)
{
	SPI2->CR2=0;
}
...
uint16_t W5100Class::write(uint16_t addr, const uint8_t *buf, uint16_t len)
{
	...
	ss.name = digitalPinToPinName(ss_pin);
	if(ss.name != NC){
		ss.port = get_GPIO_Port(STM_PORT(ss.name));
		ss.pin  = STM_LL_GPIO_PIN(ss.name);
	}else{
		ss.port = NULL;
	}
	while(!ss.port);
	if (chip == 51) {
		// setSS -> digitalWrite ss_pin LOW -> (p:= digitalPinToPinName ss_pin; digitalWriteFast p) -> (port := get_GP) 
		transferSpi2DmaBegin();
		for (uint16_t i=0; i<len; i++) {
			LL_GPIO_ResetOutputPin(ss.port,ss.pin);
			// setSS();
			// MSBFirst
			cmd[0]=0xF0;
			cmd[1]=addr >> 8;
			...
			cmd[3]=buf[i];
			addr++;
			transferSpi2DmaTransfer(cmd,cmd,4);
			// transf. con dma
			//resetSS();
			LL_GPIO_SetOutputPin(ss.port,ss.pin);
		}
		transferSpi2DmaEnd();
	} else if (chip == 52) {
		setSS();
...
//		}
		transferSpi2DmaBegin();
		transferSpi2DmaTransfer(cmd,NULL,3);
		transferSpi2DmaTransfer(buf,NULL,len);
		transferSpi2DmaEnd();
		resetSS();
	}
	return len;
...
uint16_t W5100Class::read(uint16_t addr, uint8_t *buf, uint16_t len)
{
	...
	if (chip == 51) {
		...
		transferSpi2DmaBegin();
		transferSpi2DmaTransfer(cmd,NULL,3);
		transferSpi2DmaTransfer(NULL,buf,len);
		transferSpi2DmaEnd();
//		SPI.transfer(cmd, 3);
//		memset(buf, 0, len);
//		SPI.transfer(buf, len);
