/// Versión 13 bluepill_w5100_ethernet RDC - Remapeo SPI1 a pines alternativos
/// (PA15, PB3, PB4, PB5)
#include <Arduino.h>
...
void loop(void) {
    ...
    if (cuenta_buffers_cargados == cuenta_buffers_vistos + 1) {
        transmite(buffer_ADC[cuenta_buffers_vistos % 2],
                  NMUESTRAS_BUFFER * NBYTES_MUESTRA);  // Enviar datos
